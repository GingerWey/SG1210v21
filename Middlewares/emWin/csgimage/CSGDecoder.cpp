                                        ...
                                        std::vector<uint8_t>& rawOutput) 
{
    ...
    int crn  = header.crn;
    int crnReal = (128 == (crn & 0xFF)) ? 128 : (crn & kCrnColorCountMask);  // v1.6: 128 special
    int pixelCount = header.width * header.height;
    int bpc  = BytesPerColor(cm);
    int bpi  = (0 < crnReal) ? BitsPerIndex(crnReal) : 0;

    int rawStride = (0 < crnReal) ? 1 : bpc;
    ...
    if (CompressAlgorithm::kNone == algo) {
    ...
    if (CompressAlgorithm::kDEFLATE == algo) {
    ...
    std::vector<uint8_t>& rgbaOutput) 
{
    ...
    auto cm = static_cast<ColorMode>(header.colorMode);
    int crn = header.crn;
    int crnReal = (128 == (crn & 0xFF)) ? 128 : (crn & kCrnColorCountMask);  // v1.6: 128 special        // v1.6: actual color count
    bool hasTransparent = (128 != (crn & 0xFF)) && (0 == (crn & kCrnTransparentFlag)); // v1.6: 128 special // v1.6: bit7=1 -> no transparent
    int pixelCount = header.width * header.height;
    int bpc = BytesPerColor(cm);
    ...
    for (int i = 0; i < pixelCount; ++i) {
        ...
        if (0 < crnReal) {
            ...
            if (hasTransparent && 0 == cri) {
                // CRI=0 is transparent (only when CRN bit7=0)
                isTransparent = true;
            }
            ...
            // Transparent if pixel matches palette[0] (only when CRN bit7=0)
            if (hasTransparent && static_cast<size_t>(bpc) <= palette.size()) {
...
DecoderResult CSGDecoder::DecodePicture(const uint8_t* data, size_t len) 
{
  ...
  DecoderResult result;
    ...
    int palBytes = header.PaletteByteCount();  // v1.6: masks CRN bit7
                           ...
                           ColorMode outMode) 
{
  ...
  if (nullptr == state || nullptr == pic || nullptr == lineBuf) {
    ...
    state->crn     = (128 == (pic->crn & 0xFF)) ? 128 : (pic->crn & kCrnColorCountMask);  // v1.6: 128 special
    state->bpi     = (0 < state->crn) ? BitsPerIndex(state->crn) : 0;
...
static CSG_ErrCode DecodeRLE(CSGDecoderState* st,
    ...
    if (0 <= st->pendRleType && 0 < st->pendRleCount) {
        if (1 == st->pendRleType) {
            ...
            uint8_t cri = (0 == st->pendRleType) ? 0 : st->pendRleCri;
            for (int j = 0; j < n; ++j) {
                WriteCrmPixel(output, dec, bpc, pal, cri);
                ++outPos; ++dec;
            }
            st->pendRleCount -= n;
            if (0 == st->pendRleCount) { st->pendRleType = -1; }
        }
    }
    ...
    while (dec < toDecode) {
        ...
        if (0 == frameType) {  // ZRC — transparent run (1..63 pixels)
            int n = count;
            if (n > (toDecode - dec)) {
        ...
        } else if (1 == frameType) {  // DPS — discrete pixels (1..64)
            int n = count + 1;
            int j = 0;
        ...
        } else if (2 == frameType) {  // CPS — short continuous run (1..64)
            int n = count + 1;
            uint8_t cri;
            if (0 < bpi) {
                refill(bpi);
                                  ...
                                  uint8_t* output, int toDecode) 
{
    ...
    if (0 == st->pixelsDecoded) {
        ...
        pos += 4;
        if (kCsgMaxWidth * kCsgMaxHeight < origCount) {
            return CSG_ErrCode::kErrBufferShort;
        }
        ...
        if (0 == hRaw) {
            return CSG_ErrCode::kErrMallocFailed;
        }
                             ...
                             int requestedPixels) 
{
    ...
    if (nullptr == state || nullptr == output || nullptr == state->stream) {
        return CSG_ErrCode::kErrSrcImgNull;
    }

    if (0 >= requestedPixels) {
        return CSG_ErrCode::kOk;
    }
